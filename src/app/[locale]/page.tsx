...
const CATEGORIES = ['ballBearings', 'rollerBearings', 'rollingMillBearings'] as const;
const INDUSTRIES = ['steel', 'mining', 'energy', 'automotive', 'manufacturing', 'agriculture'] as const;
const TRUST_BLOCKS = ['factoryCapability', 'qualityControl', 'exportExperience', 'responseSLA'] as const;
            ...
            {TRUST_BLOCKS.map((block) => (
              <div key={block} className="bg-white rounded-lg p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mb-4">
                  <svg className="w-6 h-6 text-blue-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={TRUST_ICONS[block]} />
                  </svg>
                </div>
                ...
                <ul className="space-y-1.5">
                  {[0, 1, 2, 3].map((i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-gray-500">
                      <svg className="w-3.5 h-3.5 text-blue-600 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
